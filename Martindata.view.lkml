view: martindata {
  derived_table: {
    sql:
      ...
      UNION ALL
      SELECT
        9 AS id, 'user 2' AS user, 'Sandra Lee' AS sales_rep, 'Cat' AS deal_name, 'Contract' AS deal_stage, 'nok' AS currency, 8250 AS deal_value
      UNION ALL
      SELECT
  ...
  # Value bucket
  measure: value_rank_measured {
    case: {
      when: {
        sql: ${value_measure} <= 1000 ;;
        label: "< 1,000"
      }
      when: {
        sql: ${value_measure} > 1000 AND ${value_measure} <= 2500 ;;
        label: "1,001 - 2,500"
      }
      when: {
        sql: ${value_measure} > 2500 AND ${value_measure} <= 5000 ;;
        label: "2,501 - 5,000"
      }
      when: {
        sql: ${value_measure} > 5000 AND ${value_measure} <= 15000 ;;
        label: "5,001 - 15,000"
      }
      when: {
        sql: ${value_measure} > 15000 AND ${value_measure} <= 50000 ;;
        label: "15,001 - 50,000"
      }
      when: {
        sql: ${value_measure} > 50000 AND ${value_measure} <= 100000 ;;
        label: "50,001 - 100,000"
      }
      when: {
        sql: ${value_measure} > 100000 ;;
        label: "> 100,000"
      }
    }
  }
...
#   }
  ...
  measure: total_value_no_f_measure {
    type: sum
    value_format_name: decimal_0
    sql: ${value} ;;
    html: {{rendered_value}} ;;
